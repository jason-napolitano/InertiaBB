<template>
  <section v-if="links.length > 3">
    <div class="flex flex-wrap -mb-1">
      <template v-for="(link, k) in links" :key="k">
        <span
          v-if="link.url === null"
          class="no-underline mr-1 mb-1 px-2 py-1 text-sm text-charcoal bg-tin border bg-gradient cursor-default"
          v-html="link.label"
        />
        <span
          @click="goTo(link.url)"
          v-else
          class=""
          :class="`no-underline mr-1 mb-1 px-2 py-1 text-sm border-none bg-gradient cursor-pointer ${
            link.active
              ? 'bg-deepocean opacity-80 text-white'
...
// go to page with smooth scroll effect
const goTo = (route) => {
  Inertia.get(route)
  window.scroll(0, 0)
}
